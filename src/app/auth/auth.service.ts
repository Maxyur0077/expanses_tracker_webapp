...
export class AuthService {
      ...
      this.http.post(this.apiUrl + 'USER/SIGN_UP', body).subscribe(
        (res: any) => {
          if (res) {
            ...
            this.isAuth = true;
            this.saveAuthDataonLocalStorage(
              res.data.expiredToken,
              res.data.userId
            );
  ...
  onLogin(body: any): Promise<boolean> {
    return new Promise<boolean>((resolve, reject) => {
      this.http.post(this.apiUrl + 'USER/LOGIN', body).subscribe(
        ...
        (error) => {
          this._snackBar.open('Login Failed', '', { duration: 3000 });
          this.isAuth = false;
          reject(error);
        }
      );
  ...
  getAllSaveData() {
    return this.http.get(
      this.apiUrl +
        'GET_SAVE_DATA/' +
        sessionStorage.getItem('Id')?.split(' ')[1]
    );
  }
  ...
  updateUserData(id: string, body: any) {
    ...
    this.http
      .post(this.apiUrl + 'UPDATE_SAVE_DATA/' + id, body)
      .subscribe((result) => {
        // console.log(result);
      });
  }
